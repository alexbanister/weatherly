...
export default class Header extends React.Component {
  ...
  componentDidMount() {
    if (localStorage.cityTrie) {
      cities.root = JSON.parse(localStorage.getItem('cityTrie'));
    } else {
  ...
  handleChange(e) {
    const suggestions = cities.getWords(e.target.value).slice(0, 10);
    this.setState({
  ...
  handleAutocomplete(word) {
    ...
    localStorage.setItem('cityTrie', JSON.stringify(cities.root));
