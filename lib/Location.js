...
export default class Location {
  ...
  // update(returnCall) {
  //   // console.log(typeof returnCall);
  //   // console.log(`http://api.wunderground.com/api/${this.key}/conditions/hourly/forecast10day/q/${this.query}.json`);
  ...
  currentConditions() {
    return {
      ...
      icon: this.data.current_observation.icon,
  ...
  hourForecast() {
    ...
    for (let i = 0; i < 7; i++) {
      sevenHour.push({
        ...
        icon: this.data.hourly_forecast[i].icon,
      });
    }
    return sevenHour;
  }
